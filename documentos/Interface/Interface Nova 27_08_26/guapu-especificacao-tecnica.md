...
| Nota de transparência (corpo) | Inter | 13px | 400 | 1.65 | normal | `#5A4322` sobre `--dourado-tinta` |
...
> **Atualizado (revisão da orientadora, ago/2026):** o parágrafo de introdução e a nota de transparência foram encurtados, e os blocos "Como usar" / "O que esperar" foram **removidos da tela inicial** para reduzir a poluição visual, especialmente em telas pequenas (celular). Esse conteúdo não desaparece do projeto — ele deve migrar para um manual resumido / card de ajuda, hospedado na página do macroprojeto (fora do escopo desta interface). Ver item pendente na seção 5.5.

| Elemento | Texto |
|---|---|
| Eyebrow | `Assistente de IA · INT 5224` |
| Título (h1) | `O cuidado no processo de viver humano II: a condição cirúrgica` |
| Parágrafo de introdução | `Olá, eu sou o Guapu, o tutor inteligente da disciplina INT 5224, do curso de Graduação em Enfermagem da UFSC. Estarei aqui para facilitar sua jornada de aprendizagem sobre o cuidado de enfermagem ao paciente cirúrgico.` |
| Nota de transparência — rótulo em negrito | `Atenção:` |
| Nota de transparência — corpo | `o uso do tutor inteligente não substitui o raciocínio clínico, a leitura dos conteúdos na íntegra ou a orientação docente.` |
| Label do divisor | `escolha uma opção` |

~~"Como usar" / "O que esperar"~~ — removidos desta tela (ver nota acima).
| Label do divisor | `escolha uma opção` |
...
- **Manual resumido / card de ajuda** — conteúdo que antes vivia em "Como usar" e "O que esperar" na tela inicial. Por decisão da orientadora do projeto, esse conteúdo sai da interface do Guapu e deve migrar para a página do macroprojeto (fora do escopo deste app), incluindo a história do Guapu. Ainda não redigido — pendência externa a este documento, não é uma lacuna de implementação da interface em si.
...
<HeroCard>                        ← substituído após a 1ª mensagem
  ...
  parágrafo de introdução (curto — saudação do Guapu, ver seção 5.2)
  <TransparencyNote>               fundo dourado-tinta, ícone "i" (texto curto, sem <HowToUse>)
...
- `ActionGrid`: de 2 colunas para 1 (o `HowToUse` foi removido da tela, ver seção 5.2 — não precisa mais de regra responsiva própria).
...
- [x] Menu inicial encurtado conforme revisão da orientadora (ago/2026) — "Como usar"/"O que esperar" removidos, introdução e nota de transparência reescritas mais curtas
- [x] Símbolo `<GuapuMark />` corrigido — pivot de rotação no centro real da forma (54,52) e escala 1.0, corrige o vazamento do bico para fora do círculo
