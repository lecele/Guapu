...
| 7. Homologação e liberação | Em homologação interna | Bateria técnica publicada 8/8 aprovada; roteiro interno e evidência em `documentos/QA/Fase_7/` | Requer execução e aprovação funcional de Leonardo, atuando como cliente; não encerrar sem esse registro |
...
- Verificação ao vivo em 30/08/2026: bateria específica de referências e bloqueio passou em **6/6** no domínio publicado, cobrindo duas perguntas do plano vigente, duas do glossário DOCX e duas consultas ao plano antigo. As fontes foram compatíveis, as referências apareceram nos casos fundamentados e o documento antigo retornou `NO_RELEVANT_CONTEXT` com fallback controlado.
