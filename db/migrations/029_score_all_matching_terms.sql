-- Um documento que corresponde a vários termos da consulta deve superar um
-- candidato genérico que coincidiu semanticamente com apenas uma ideia.
-- Soma os escores por termo, preservando a busca semântica como complemento.
...
AS $$
    ...
    lexical_candidates AS MATERIALIZED (
        SELECT candidates.id, candidates.content, candidates.source, candidates.metadata,
               ...
               (0.50 + sum(candidates.term_score))::FLOAT AS lexical_score
        FROM (
            SELECT d.id, d.content, d.source, d.metadata,
                   ts_rank_cd(
